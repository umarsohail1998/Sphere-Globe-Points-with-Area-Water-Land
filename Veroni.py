def main():
    ...
    radius = int(input("please input your desired radius for sphere: "))
    no_points = int(input("please input your desired number of points: "))
    size_limit = int(input("Enter file Size-limit: "))
    ...
    def make_dict(generator_index, polygon_area, area_type, list_of_coordinates):
        od = OrderedDict()
        od['area_id'] = generator_index
        od['area_contained'] = polygon_area
        od['area_type'] = area_type
        od['latitude_and_longitude'] = list_of_coordinates
        return od
...
#             writer.writerow([generator_index,polygon_area(voronoi_region),area_type,list_of_coordinates])
            ...
            row.append(make_dict(generator_index, polygon_area(voronoi_region), area_type, list_of_coordinates))

        with open(f'JSON_Files/File{count}.json','w') as f1:
            json.dump(row, f1)
        if is_break:
            break
        start = end
